...
def build_search_links(preferences: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    query = " ".join(part for part in (role, skills) if part).strip()
    ...
    return [
        ...
        {
            "name": "Indeed India",
            "url": f"https://in.indeed.com/jobs?q={encoded_query}&l={encoded_location}&fromage=7&sort=date",
            "description": "Recent jobs in India matching your role and skills."
        },
        {
            "name": "Naukri",
...
async def fetch_adzuna_india_jobs(preferences: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    params = {
        "app_id": app_id,
        "app_key": app_key,
        "what": " ".join([preferences["role"], *preferences.get("skills", [])[:5]]),
...
async def search_jobs(request: JobSearchRequest):
    ...
    jobs = await fetch_adzuna_india_jobs(preferences_dict)
    return {
        "jobs": jobs,
